...
const SECURITY_HEADERS = {
  ...
  'Content-Security-Policy': "default-src 'none'; style-src 'unsafe-inline'; img-src 'self'; frame-ancestors 'none'; form-action 'self'; base-uri 'self'",
  ...
  async fetch(request) {
    ...
    if (path === '/robots.txt') {
      return new Response(robotsTxt(), {
        headers: withSecurityHeaders({ 'Content-Type': 'text/plain; charset=utf-8', 'Cache-Control': 'public, max-age=86400' }),
      });
    }

    if (path === '/sitemap.xml') {
      return new Response(sitemapXml(), {
        headers: withSecurityHeaders({ 'Content-Type': 'application/xml; charset=utf-8', 'Cache-Control': 'public, max-age=3600' }),
      });
    }

    if (path === '/') {
      return new Response(homePage(), {
        headers: withSecurityHeaders({ 'Content-Type': 'text/html; charset=utf-8', 'Cache-Control': 'public, max-age=300' }),
      });
    }
